<script module lang="ts">
	// Verbatim port from svelte-bits (https://sveltebits.xyz/backgrounds/dot-field,
	// registry: dot-field) — the Svelte-native sibling of reactbits.dev's Dot
	// Field. Left unmodified so it stays a faithful copy of the upstream
	// component; theme-appropriate colours are passed in reactively from the
	// call site instead (gradientFrom/gradientTo/glowColor are read fresh on
	// every animation frame since $props() destructuring compiles to live
	// getters, so simply passing different values in from the parent when the
	// theme changes is enough — no internal theme-awareness needed here).
